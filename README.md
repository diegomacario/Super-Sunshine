...
### 1) Setup
In a scene description, there are three things that need to be specified before anything else. The first two are the *__dimensions__* of the image that will be generated by the ray-tracer, and the third one is the *__filename__* of said image. These settings are specified with the following commands:
 ```sh
 size width height
 output filename.png
 ```
